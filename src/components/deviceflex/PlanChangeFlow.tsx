...
export function PlanChangeFlow({
  ...
  return (
    <Overlay open onClose={onClose} labelledBy="pc-title" className="max-w-2xl">
          ...
          <div className="space-y-5">
            ...
            {overCapacity && (
              <p className="flex items-start gap-2 rounded-xl bg-[#FDF3F5] p-3 text-sm text-[#C70032]">
                ...
                {tier.name} covers {capacity} device{capacity > 1 ? "s" : ""} and you&rsquo;ve kept{" "}
                {selected.length}. Uncheck {selected.length - capacity} to continue.
              </p>
            )}
              ...
              <div>
                ...
                <p className="att-small mt-1">
                  ...
                  {OPEN_ENROLLMENT.active && (
                    <>
                      {OPEN_ENROLLMENT.label} is running until {formatDate(OPEN_ENROLLMENT.endsOn)},
                      so age isn&rsquo;t a barrier right now.
                    </>
                  )}
                </p>
                <div className="mt-3 grid gap-2">
          ...
          <div>
            ...
            {dropped.length > 0 && (
              <p className="mt-4 flex items-start gap-2 rounded-xl bg-[#FDF3F5] p-3 text-sm text-[#C70032]">
                ...
                <span>
                  <b>
                    {dropped.map((d) => d.name).join(", ")} {dropped.length > 1 ? "lose" : "loses"}{" "}
